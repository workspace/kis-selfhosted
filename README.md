...
한국투자증권 Open API 기반 트레이딩 플랫폼을 Docker로 셀프호스팅합니다.
...
- 도메인 + Nginx Proxy Manager (또는 다른 리버스 프록시)
- 한국투자증권 Open API 키 ([서비스 신청](https://apiportal.koreainvestment.com/about-howto))
...
| 항목 | 값 |
|------|-----|
| Domain | `kis.example.com` |
| Forward | `http://<server-ip>:8080` |
| SSL | Let's Encrypt |
| WebSocket | ON |
...
{
  "mcpServers": {
    "kis-backtest": { "url": "https://kis.example.com/mcp/backtest" },
    "kis-trading": { "url": "https://kis.example.com/mcp/trading" }
  }
}
```
...
| `KIS_HTS_ID` | - | HTS ID |
| `KIS_ACCT_STOCK` | - | 증권계좌 (8자리) |
| `KIS_ACCT_FUTURE` | - | 선물옵션계좌 (8자리) |
| `KIS_PAPER_STOCK` | - | 모의 증권계좌 |
| `KIS_PAPER_FUTURE` | - | 모의 선물옵션계좌 |
| `KIS_PROD_TYPE` | - | 상품코드 (기본: 01) |

## 로컬 개발

Docker 없이 로컬에서 개발하려면 원 프로젝트 README의 설정 가이드를 참고하세요.

```bash
# Strategy Builder
cd strategy_builder && ./start.sh

# Backtester
cd backtester && ./start.sh
```

---

<details>
<summary>원 프로젝트 README 보기</summary>
...
│   ├── kis_auth.py              # 인증 공통 함수
│   ├── domestic_bond            # 국내채권
│   │   ├── domestic_bond_functions.py        # (REST) 통합 함수 파일
│   │   ├── domestic_bond_examples.py         # (REST) 실행 예제 파일
